################################################################################
# Name:    Kurt Lancaster
# Class:   CS2318-00? (Assembly Language, Fall 2018)
...
####################(7)####################
					addi $a0, $sp, 97
					jal GetOneIntByVal
					lw $t0, 52($sp)		#$t0 has valsToDo
					sub $t0, $t0, $t1	#$t0 is valsToDo-i
					sll $t0, $t0, 2		#$t0 is (valsToDo-i)*4	
					add $t0, $sp, $t0	
					sw $v0, 0($t0)			
					j endI1
#         else // i is even
...
GetOneIntByAddr:
#   cout << prompt;
					move $t0, $a0		#intVarToPutInPtr => $t0
					move $a0, $a1
					li $v0, 4
					syscall
