...
# prolog: 
			addiu $sp, $sp, -88
			sw $ra, 84($sp)		#$ra needs 4 bytes at bottom 
			sw $fp, 80($sp)		#$fp same space needed after return address
			addiu $fp, $sp, 88	#$fp to the sp(old) location
# body:
...
# prolog:
			# no stack frame needed
# body:
			sw $a0, 0($sp)				# save $a0 (= &a1[0]) in caller's frame
			sw $a2, 8($sp)				# save $a2 (= &a2[0]) in caller's frame
...
begFor1:
			# sum1 = sum1 + a1[i]
			lw $a0, 0($sp)				# reset $a0 to &a1[0] as received
			sll $t5, $t5, 2				# temporarily scale i 4 times
...
begFor2:
			# sum2 = sum2 + a2[i]
			lw $a2, 8($sp)				# reset $a2 to &a2[0] as received
			sll $t5, $t5, 2				# temporarily scale i 4 times
...
# body:
			...
			sw $t0, 20($sp)				# save $t0 prior call (caller saved)
			...
			jal Sum2Arrays
			move $t1, $v0
			lw $t0, 20($sp)				# restore $t0 after call (caller saved)
