...
#			endPtr1 = iArr + used;
			la $t5, iArr
			sll $t8, $t0, 2				#sll to multiply used by 4  store in $t8
			add $t8, $t8, $t5			#multiplying 4 * 4
#			hopPtr1 = iArr; 
			la $t1, iArr				
#			goto FTest1;
			j FTest1
...
#			if (hopPtr1 >= endPtr1 - 1) goto else3;
			...
			bge $t1, $t5, else3
begI3:
#			cout << *hopPtr1 << "  "; 	#cout<<*(hopPtr1 + 0) << " " is the same thing or hopPtr[0] too
			li $v0, 1					#printing integer
			lw $a0, 0($t1)				#load word directly into the address 0 offset ^^
			syscall
			li $v0, 11
			li $a0, ' '
			syscall
...
FTest1:			blt $t1, $t8, begF1				#if hopPtr < endPtr goto target begF1
endF1:

#			hopPtr1 = iArr;
			la $t1, iArr						#now $t1 has base address of array
#			endPtr1 = iArr + used;
			sll $t8, $t0, 2
			add $t8, $t8, $t1

#			minInt = *iArr;					#minInt is just a normal integer
			lw $a2, 0($t1)						#take iArr
#			maxInt = minInt;
			move $a3, $a2
#			++hopPtr1;
			addi $t1, $t1, 4
...
#			if (*hopPtr1 >= minInt) goto else4;
			lw $t5, 0($t1)						#have to load word because otherwise youd compare address w/ int...
			bge $t5, $a2, else4
begI4:
#			minInt = *hopPtr1;
			move $a2, $t5
